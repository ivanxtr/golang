...
func TestTrasnferTx(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		go func() {
			result, err := store.TransferTx(context.Background(), TrasnferTxParams{
				FromAccountID: account1.ID,
				ToAccountID: account2.ID,
				Amount: amount,
			})
			
			// sending back from go rutines to main channel
			errs <- err
			results <- result
		}()
	}
...
func TestTrasnferTxDeadLock(t *testing.T) {
	...
	for i := 0; i < n; i++ {
		...
		go func() {
			_, err := store.TransferTx(context.Background(), TrasnferTxParams{
